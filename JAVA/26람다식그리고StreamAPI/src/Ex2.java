...
public class Ex2 {
	...
	*/
	...
	//순서2. 중간연산 - 필터링 
	//필터링  : .filter(람다식) 메소드를 사용하여 각 성적을 검사하고, 60점 이상인 성적만 남김
	...
	/*
		...
		-------------------------------------------
	*/	
	//순서2-1. 변환 연산  - IntegerStream통로를 IntStream통로로 변환 
	// mapToInt()메서드 
	// - 스트림에 있는 객체(요소)들을 정수형(int)으로 변환하여 IntStream통로에 담아
	//    IntStream통로를 반환 하는 메소드
	// - 일반적으로 객체가 저장된 스트림을 처리할때, 숫자 계산이나 통계처리를 하기위해
	//   객체(요소)들을  기본자료형 스트림(IntStream, DoubleStream, LongStream)
	//   으로 변환할때 사용되는 메소드 
	//람다식
	//   score -> score
	//   (score) -> { return score;  };
