...
/*
	...
	Files클래스	
		...
		- 정적메소드로 구성되어 있기때문에 File클래스처럼 객체를 만들필요하 없다.
...
public class FileTest03 {
	public static void main(String[] args) throws Exception {
		//주제 : File클래스를 이용해 C:\Temp 디렉토리에 user.txt파일을 생성하고 읽는 방법을 보여주는 예제
		try {
		...
		//주제 : C:\Temp 디렉토리에 images디렉터리와 file1.txt, file2.txt, file3.txt파일을 생성하고
		//     Temp디렉터리에 있는 내용을 출력하는 예제.
		...
		//File객체를 생성했다고 해서 파일이나 디렉터리가 생성되는 것은 아니다.
		//그리고 경로에 실제 파일이나 디렉터리가 없더라도 예외가 발생하지 않는다.
		//파일이나 디렉토리가 실제 있는지 확인하고 싶다면 File객체를 생성하고 나서 exists()메소드를 호출해보면된다
		
		//존재하지 않으면 디렉터리 또는 파일 생성
		...
		//참고.
		// File[]  listFiles() 메소드 
		// -> 디렉터리에 포함된 파일 및 서브 디렉토리 목록 전부를  File배열에 담아 리턴 해줍니다.
		File[] contents = temp.listFiles();
		
		//contents File배열에 저장된 File객체의 갯수만큼 반복해서 얻어 Temp폴더 아래의 
		//파일의 정보들을 반복해서 출력
		for( File file : contents ) {
			
			if(file.isDirectory()) {//디렉터리냐?
