...
public class Collections01 {
	public static void main(String[] args) {
		...
		//자식 HashSet클래스의 객체 생성 후 주소번지 저장
		Set set = new HashSet();
		
		//HashSet가변길이 배열에 저장된 객체(데이터)의 갯수 반환
		System.out.println("HashSet배열에 저장된 객체의 갯수->"
		                    + set.size());
		
		//HashSet가변길이 배열에 객체를 추가하기 위해 add메소드 사용!
		set.add("하나");
		set.add(2);//오토박싱되어 new Integer(2);객체가 추가되어 저장되는것이다
		set.add(3.42);//오토박싱되어 new Double(3.42);객체가 추가되어 저장되는것이다
		set.add("넷");
		set.add("five");
		...
		System.out.println("HashSet배열에 저장된 객체의 갯수->"
		                    + set.size());		
		//HashSet가변으로늘어나는 배열에 저장된 어떤 객체들이 있는지 확인을 위해
		//toString()메소드를 호출해 전체 배열을 문자열로 변환해서 반환해서 출력
		System.out.println( set.toString()  );
		//[2, 6, 넷, 하나, 3.42, five]
		
		//HasSet가변 길이 배열에는 add메소드를 이용하여 
		//추가된 순서대로 객체가 저장되지 않고 랜덤으로 뒤섞여서 저장됩니다.
