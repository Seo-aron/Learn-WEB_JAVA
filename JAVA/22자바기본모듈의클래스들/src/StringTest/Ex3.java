...
//new MemberDTO("홍길동","서울시 강남구 테헤란로 123/101호");
...
class MemberDTO{
    ...
    //address변수에 저장된 회원의 전체 주소 전체문자열 중에서 
    //마지막에 작성된 '/'가 포함되어 있는지 검사를 위해
    //'/'문자의 index위치 번호를 찾음
		
				//"광주시 북구 분문로 112/505호".lastIndexOf('/');
				// 012345678910     16
		int lastIndex = address.lastIndexOf('/'); //16을 반환
			
	//상세주소가 있을 경우 전체 주소에서 '/'문자다음부터 끝까지의주소를 잘라서 반환	
		// '/'가 존재하고 그다음에 문자가 있을 경우
		if( lastIndex != -1 && lastIndex + 1  < address.length()  ) {
			 //lastIndex가 -1 이아니면 '/'가 존재함을 의미
			 //lastIndex + 1은 '/' 다음 문자의 인덱스
			 //lastIndex + 1이 address.length()보다 작으면 '/'다음에 문자가 존재함을 의미
			
	 //전체 주소 문자열에 적힌 '/'다음부터 문자열의 끝까지 잘라서 반환(상세주소만 잘라서 반환)
...
public class Ex3 {
	public static void main(String[] args) {
		...
		//회원 수를 정의 
		final int MEMEBER_COUNT = 5; //5명 
		
		//데이터베이스에 저장된 5명의 정보를 조회해서 가져와 
		//한명씩 MemberDTO객체에 저장 할것이다
		//new MemberDTO(); X 5
		
		//MemberDTO객체5개를 저장시킬 칸이 5칸인 배열 생성
		MemberDTO[] memberArray = new MemberDTO[MEMEBER_COUNT];
							//[ null,  null,  null, null,  null ];
						    //   0      1      2      3     4 
		
		//데이터베이스에서 회원정보 조회(여기서는 예시로 고정된 조회된 값 사용)
		memberArray[0] = new MemberDTO("홍길동","서울시 강남구 테헤란로 123/101호");
		memberArray[1] = new MemberDTO("김철수","부산시 해운대구해변로 456/202호");
		...
		//각 회원(MemberDTO객체)의 이름과 상세주소를 출력
