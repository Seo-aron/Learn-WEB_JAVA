...
/*
   this 키워드
   - This class설계도 내부에 this를 작성하면 
     this는 현재 생성한 객체 자신을 가리키는 참조변수 
     
     예)  new This(); <- 0x16 객체메모리의 주소번지
   ...
   - 사용목적
     ...
     - 생성된 객체의 메소드나 변수에 접근: 
       객체자신의 메소드나 변수에 접근할떄 this를 사용할수 있다
   
   
   this()
   - 자바에서 생성자 내부에서 다른 생성자를 호출할때 사용되는 키워드입니다.  
   - 같은 클래스 내부의 생성자들끼리 코드를 재사용할수 있도록 돕는 역할을 하는 키워드.
   
   this()  주요 특징
   1.다른 생성자 호출
   2.생성자 내부에 작성하는 this()는 생성자의 가장 첫줄에 작성해야합니다.
     그렇지 않으면 컴파일 에러가 발생합니다
   3.오버로딩된 생성자 호출
     : this()를 이용해 다른생성자를 호출할때 전달하는 인수에 따라 호출될 생성자가 결정됨
...
class Car2{
	...
	//생성자 - 두개의 객체변수를 모두 초기화할 생성자
	//생성자 만드는 단축키 alt + shift + s  o
	//생성자 만들때 메뉴 순서
	//Source메뉴 클릭 ->  ... using Field 클릭 
	...
	public Car2(int year) {
		...
		this("검정색",year); //참고. 전달하는 값의 갯수에 따라 
						    //     만약 갯수가 2개면 
							//     매개변수가 2개인 다른 생성자를 호출하게됨		
	}
	
	//자동차 객체 정보를 출력하는 메소드
	public void printInfo() {
									 //생성된 Car2객체 메모리 내부의
									 // color객체변수의 값을 불러와서 출력
