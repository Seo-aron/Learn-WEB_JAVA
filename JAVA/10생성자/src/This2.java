...
class Book{
	...
	public Book() {
		...
		//매개변수가 3개인 생성자 호출시 위 값 3개 전달 하여 초기화
		this("제목 없음", "작자 미상", 0);
	}
...
//		     출판연도 : 0
		...
		//제목과 저자만 설정하는 생성자를 호출해서 Book객체 생성
...
//			 출판연도 : 0
		...
		//모든 객체변수들(제목, 저자, 출판연도)를 설정하는 생성자를 호출해서 Book객체 생성
...
//			 출판연도 : 1949
		...
		/*
			요약 : this()는 같은 클래스의 생성자내부에서 다른 생성자를 호출하는데 사용됨
			      중복된 객체변수 초기화 코드를 줄이고,
			      생성자 간의 관계를 체계적으로 유지할수 있습니다.
			      생성자 간의 코드 재사용을 통해 유지보수성과 가독성을 높이는데 도움이됨
			      
			결론 : this()의 사용법을 이해하고, 생성자 오버로딩을 활용하여
			      객체변수의 값을 초기화를 효율적으로 관리하는 방법을 배울수 있다.
