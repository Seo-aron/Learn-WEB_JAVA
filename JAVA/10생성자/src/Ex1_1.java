...
//	final int APPLE_PRICE = 1000;
	...
	*/
	...
	//나중에 FruitSeller클래스의 객메 메모리 생성후 
	//각 객체변수 3개의 값을 저장할 기능의 메소드 추가
						   //사과 1개당 가격,  //현재 보유한 사과갯수, //판매수익이자 잔고 
...
public class Ex1_1 {
	public static void main(String[] args) {
		...
		// 해결방법 사용 : 
		// 과일장수1,2 객체를 생성하고 나서, 
		// 객체변수3개를 추가한 메소드로 초기화합니다.
		
		//과일장수1 객체 생성
		FruitSeller seller1 = new FruitSeller();
		//생성된 과일장수 1객체의 객체변수들의 값을 저장하기 위해  
		//-> 추가한 initMembers메소드 호출
        ...
        //생성된 과일장수 2객체의 객체변수들의 값을 저장하기 위해
		//-> 추가한 initMembers메소드 호출
		// 사과 1개당 가격 : 10,00 ,  현재 보유한 사과갯수: 20개, 판매수익이자 잔고 :0원
		seller2.initMembers(1000, 20, 0);
			
		//1명의 과일 구매자 객체 생성
		FruitBuyer buyer = new FruitBuyer();		
		//과일구매자 객체는 과일장수1객체에게 4500원을 지불하고 사과 구매
		buyer.buyApple(seller1, 4500);
		
		//과일구매자 객체의 현재 객체변수에 저장된 값들의 상황 출력
		System.out.println("--------과일 구매자의 현재 상황--------");
		buyer.showBuyResult();
		
		//과일장수1객체의 현재 객체변수에 저장된 값들의 상황 출력
		System.out.println("--------과일 장수1의 현재 상황--------");
		seller1.showSaleResult();
		
		
		//1명의 과일구매자 객체가 과일장수2객체에 2000원어치 사과 구매하기
		...
		//과일장수2객체의 현재 객체변수에 저장된 값들 출력
		System.out.println("-----------과일 장수2객체의 현재 상황-------");
				   ...
				   객체 메모리 내부의  APPLE_PRICE상수의 final선언이 사라진점이 문제가됨
				    ...
				    final선언이 사리진점이 왜 문제가 될까?
				     : 클래스 설계시 과일장수가 판매하는 사과 한개당 가격은 변경되지 않음을 가정해 두고
				       ...
				       메소드라는 것은 만들어 놓고 여러번 호출할수 있으므로
				       initMember메소드를 여러번 호출한다면
				       객체메모리의 변수값 초기화 하는작업의 실수를 범할수도 있기때문에
				       결론은 객체메모리 생성하는 동시에 단 한번만 호출되는 생성자를 이용해
