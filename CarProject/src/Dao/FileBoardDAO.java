...
public class FileBoardDAO {
	...
	public ArrayList boardListAll() {
		...
		try {
			...
			while (rs.next()) {
				FileBoardVo vo = new FileBoardVo(rs.getInt("b_idx"), rs.getInt("b_group"), rs.getInt("b_level"),
						rs.getInt("b_cnt"), rs.getString("b_id"), rs.getString("b_pw"), rs.getString("b_name"),
						rs.getString("b_email"), rs.getString("b_title"), rs.getString("b_content"),
						rs.getDate("b_date"), rs.getString("ofile"), rs.getString("sfile"), rs.getInt("downcount")
	...
	public ArrayList boardList(String key, String word) {
		...
		if (!word.equals("")) {
			...
			if (key.equals("titleContent")) {
				...
				sql = "select * from Fileboard " + "where b_title like  '%" + word + "%' " + "OR b_content like '%"
						+ word + "%' " + "order by b_group asc";
...
//insert에 성공한 새 글의 글번호를 조회해서 반환 하는 기능도 있음 
	...
	public FileBoardVo boardRead(String b_idx) {
		...
		try {
			...
			if (rs.next()) {
				...
				vo = new FileBoardVo(rs.getInt("b_idx"), rs.getInt("b_group"), rs.getInt("b_level"), rs.getInt("b_cnt"),
						rs.getString("b_id"), rs.getString("b_pw"), rs.getString("b_name"), rs.getString("b_email"),
						rs.getString("b_title"), rs.getString("b_content"), rs.getDate("b_date"), rs.getString("ofile"),
						rs.getString("sfile"), rs.getInt("downcount"));
			}
		} catch (Exception e) {
	...
	public boolean passCheck(String b_idx_, String password) {
		...
		try {
			...
			// 행의 모든 열값을 조회
			String sql = "select * from fileboard " + " where b_idx=? and b_pw=? " + " order by b_idx desc";
			pstmt = con.prepareStatement(sql);
			// ?설정
	...
	public int updateBoard(String idx_, String email_, String title_, String content_) {
		...
		try {
			...
			String sql = "update fileboard set " + "b_email=?, b_title=?, b_content=? " + "where b_idx=?";
			pstmt = con.prepareStatement(sql);
...
//규칙3. 추가(INSERT)하는 답변글은 주글(부모들)의 b_level열값을 가져와 +1을 해서 insert한다
	...
	public void replyInsertBoard(String super_b_idx, String reply_id, String reply_name, String reply_email,
		...
		try {
			...
			sql = "SELECT b_group, b_level FROM fileBOARD " + "WHERE b_idx=?";
			pstmt = con.prepareStatement(sql);
