...
public class FileBoardService {
	...
	public int serviceUpdateBoard(String idx_, String email_, String title_, String content_) {
	...
	public void serviceReplyInsertBoard(String super_b_idx, String reply_id, String reply_name, String reply_email,
		...
		fileboarddao.replyInsertBoard(super_b_idx, reply_id, reply_name, reply_email, reply_title, reply_content,
				reply_pass);
	...
	public synchronized int serviceInsertBoard(HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		...
		/*
		 * 참고. 클라이언트가 첨부파일을 업로드후 파일 이동 요청할 때, 클라이언트의 요청은 서버에서 별도의 스레드로 처리됩니다. 일반적으로 웹
		 * 애플리케이션에서는 각 클라이언트의 요청이 독립적인 한명의 클라이언트 요청에 관한 스레드 에서 처리되어, 동시에 여러 클라이언트의 요청을
		 * 병렬로 처리할 수 있습니다.
		 * 
		 * 스레드 처리 방식 클라이언트 요청: 클라이언트가 웹 서버에 파일 업로드 요청을 보내면, 웹 서버는 해당 요청을 처리하기 위해 새로운
		 * 스레드를 생성하거나 기존 스레드를 사용하여 요청을 처리합니다.
		 * 
		 * 서버의 스레드: 이 스레드는 클라이언트의 요청을 받아서 해당 메서드(예: serviceInsertBoard)를 실행하고, 이 메서드 내에서
		 * synchronized(this) 블록이 포함된 경우, 현재 FileBoardService 인스턴스에 대한 접근이 동기화됩니다.
		 * 
		 * 동기화의 역할: 이렇게 동기화된 블록 안에서는 동시에 여러 스레드가 접근하지 못하도록 제한하므로, 파일 업로드와 같은 중요한 작업이 동시에
		 * 여러 스레드에서 실행될 경우 발생할 수 있는 데이터 손상이나 상태 불일치 문제를 예방할 수 있습니다.
		 * 
		 * 예시 만약 두 명의 클라이언트가 동시에 temp폴더에 파일을 업로드후 파일 이동 하는 경우, 첫 번째 클라이언트의 요청이
		 * synchronized(this) 블록을 실행 중일 때, 두 번째 클라이언트의 요청은 첫 번째 요청이 끝날 때까지 대기하게 됩니다.
		 * 결론적으로, 클라이언트의 요청은 스레드로 처리되며, synchronized(this)는 이러한 스레드가 공유 자원에 동시에 접근하는 것을
		 * 방지하는 역할을 합니다.
		 */
		synchronized (this) {
			...
			if (sfile != null && sfile.length() != 0) {
				...
				/*
				 * 문법 public static void moveFileToDirectory(File srcFile, File destDir, boolean
				 * createDestDir) 매개변수 설명 File srcFile: 이동할 원본 파일을 나타내는 File 객체입니다. 이 파일은 존재해야
				 * 하며, 이동할 파일의 경로를 지정해야 합니다.
				 * 
				 * File destDir: 파일을 이동할 대상 디렉토리를 나타내는 File 객체입니다. 이 디렉토리는 존재해야 하며, 존재하지 않을 경우
				 * createDestDir 매개변수가 true로 설정되어 있으면 새로 생성됩니다.
				 * 
				 * boolean createDestDir: 이 매개변수는 대상 디렉토리가 존재하지 않을 때 생성할지를 결정하는 부울 값입니다. true일
				 * 경우, 대상 디렉토리가 없으면 새로 생성합니다. false일 경우, 대상 디렉토리가 존재하지 않으면 IOException이 발생합니다.
				 * 
				 */
			}
		} // ----------------------synchronized
	...
	private synchronized Map<String, String> upload(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			/*
			 * fileboardwrite.jsp 파일업로드 요청하는 디자인 페이지에서 첨부한 파일1개와 입력한 파라미터 여러개의 정보들
			 * request객체에서 꺼내와서 각각의 DiskFileItem객체들에 저장한 후 각각의 DiskFileItem객체들을 ArrayList배열에
			 * 추가 하게 됩니다. 그후 ~ ArrayList배열 자체를 반환 해 줍니다.
			 */
	...
	// Download.do
	// 다운로드 횟수 증가
	public void serviceDownload(HttpServletRequest request, HttpServletResponse response) throws IOException {
