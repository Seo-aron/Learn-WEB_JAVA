...
<body>
	...
	<h2>여러가지 문자열 관련 처리 함수들</h2>
						 <%-- fn:length함수 호출시  문자열을 전달하면  
						           문자열의 전체문자갯수반환 --%>
	fn:length(title1) -> ${  fn:length(title1)  } <br> <%-- 12 --%>
	
						  <%-- fn:toUpperCase함수 호출시 문자열을 전달하면  
						             소문자들을 모두 대문자로 변경한 전체문자열을 반환 --%>
    fn:toUpperCase(title1) ->  ${ fn:toUpperCase(title1)  } <br> <%--HELLO WORLD!--%>


						<%-- fn:toLowerCase함수 호출시 문자열을 전달하면  
						         대문자들을 모두 소문자로 변경한 전체문자열을 반환 --%>
 ...
 <%-- fn:substring함수 호출시  첫번쨰 전달 값으로 자를 전체 문자열을 전달하고,
    ...
    fn:substring(title1, 3, 6) -> ${ fn:substring(title1, 3, 6)    } <br>
								...
								<%--  trim함수 호출시  문자열을 전달하면  
								           양쪽에 빈공백이 있으면 양쪽 공백만 제거후 
								           제거한 문자열을 반환해줌  --%>
    fn:trim('  안녕 하   세요    ') -> ${ fn:trim('  안녕 하   세요    ') } <br> <%-- "안녕 하 세요" --%>
...
<%-- replace함수 호출시  첫번째 전달값으로 전체 문자열을 전달하고,
 ...
 --%>
	...
	<%-- indexOf메소드 호출시  전체 문자열을 첫번째 매개변수로 전달하고,
		                                     찾을 문자열을 두번째 매개변수로 전달하면
		                                     전체 문자열의 앞왼쪽부터 찾는데 만약 찾게 되면 찾은 문자열에서의 첫 문자의 index위치 번호를 반환합니다. 
