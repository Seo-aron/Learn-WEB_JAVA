...
//login.jsp페이지 내부에서 클라이언트가 요청한 전체 URL주소 중에서 
//  /pro14 <- 컨텍스트주소 : 톰캣서버가 pro14프로젝트 찾아갈 /pro14 주소 를 얻을수 있다.     
	...
	<%--방법2. 
	 자바코드로 request객체의 getContextPath메소드 호출하여 
	 login.jsp를 요청한 전체 URL중에서 /pro14 <- 컨텍스트 주소만 반환 받아 사용 --%>
	<a href="<%=contextPath%>/test01/memberForm.jsp">회원가입하러가기</a>

	<br><br>
	
	<%--방법3. EL의  pageContext내장객체를 사용한 방법
			...
			사용 예
				<a>태그를 이용해 다른 서블릿이나 JSP를 요청하는 방법은 2가지 입니다
					...
					문제점
						 ...
						 방법2는   자바코드가 HTML코드와 사용되므로 작업이 복잡해 진다는 당점 이 있다
