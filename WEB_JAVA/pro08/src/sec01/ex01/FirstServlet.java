...
public class FirstServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, 
		...
		//(HttpServletResponse객체의 sendRedirect()메소드를 호출해서 사용하는 방법)
		//-> 첫번째 서블릿을 처음 요청했지만
		//   첫번쨰 서블릿 내부 코드에서 두번째 서블릿을 
		//   웹브라우저 주소창을 거쳐서 재요청(리다이렉트 포워드)한다는 의미입니다.
		//-> 문법
		//		response.sendRedirect("재요청할 두번쨰 서블릿 매핑주소");
		
		//1. 서블릿 간의 재요청의 경우! 문자처리방식 UTF-8로 설정해야
		//   재요청을 받는 두번쨰 서블릿에서 첫번째 서블릿에서 요청받은 데이터를 공유받을때
		//   한글이 깨지지 않고 공유받아서 사용할수 있다.
